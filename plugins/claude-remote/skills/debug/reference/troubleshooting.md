...
| `API rate limit exceeded` / `403` from `api.github.com` during `mise install` | Cloud sandbox is unauthenticated; mise hit GitHub's anonymous rate limit while resolving `latest` or fetching a GitHub-backed backend. | Generate `mise.lock` locally (`mise lock --platform linux-x64,macos-arm64`), commit it, and re-run. The current `setup.sh` template uses `mise install --locked` automatically when `mise.lock` is present. If the user is on an old `setup.sh`, suggest `/claude-remote:setup` to regenerate. |
| `mise install --locked` fails with `URL not found for platform linux-x64` | Lockfile was generated only for the developer's host platform (typically macOS) and lacks Linux URLs. The template's safety net falls back to plain `mise install` for this run. | Re-run `mise lock --platform linux-x64,macos-arm64` locally, commit, and push. Until then, expect occasional rate-limit failures. |
| `mise.lock` missing in repo | No lockfile committed — every cloud session re-resolves versions via the GitHub API and risks rate limiting. | Run `mise lock --platform linux-x64,macos-arm64`, commit `mise.lock`. Optional: `mise settings lockfile=true` so future `mise install` updates the lockfile automatically. Do **not** set `[settings] locked = true` in `mise.toml` — that flag is global-scope and breaks tools outside the repo. |
...
| `setup.sh` calls `mise install` (no `--locked`) | Legacy template — predates lockfile support. Vulnerable to GitHub rate limits. | Re-run `/claude-remote:setup`; it detects this and offers to regenerate. Confirm `mise.lock` is committed afterward. |
